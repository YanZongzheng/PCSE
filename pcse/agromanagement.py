...
class AgroManagementSingleCropNoTerminate(AncillaryObject):
    """Agromanagement for running a full year cycle including
    a single cropping season.

    There are a few differences with the class AgroManagementSingleCrop:
    * a TERMINATE signal is not sent when a CROP_FINISH
      signal is received. Instead it is assumed that a TERMINATE signal
      is sent by the timer module when the `END_DATE` is reached.
    * Another difference is that the crop simulation object is
      deleted from the Engine by setting `crop_delete=True` when
      sending a CROP_FINISH signal.
    ...
    * "TERMINATE": sent when a "CROP_FINISH" signal is received.
    ...
    def _on_CROP_FINISH(self):
        "Register that crop cycle is over."
