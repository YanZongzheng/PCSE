...
from .crop_waterbalance_accumulator import CropWaterBalanceAccumulator

#-------------------------------------------------------------------------------
class WofostCWBA(SimulationObject):
    """Top level object organizing the different components of the WOFOST crop
    simulation.

    Difference with this version and the standard WOFOST is that it includes
    the CropWaterBalanceAccumulator (CWBA) module that computes water
    balance summary terms over the crop cycle. These terms are exactly the
    same as the waterbalance, but the waterbalance accumulates them over the
    entire model run instead of the crop cycle only.
    ...
    implemented as embedded simulation objects consist of:
        ...
        10. Crop water balance accumulation (self.cwba)
    ...
    **State variables:**
    ...
    cwba = Instance(SimulationObject)
    ...
    def initialize(self, day, kiosk, parvalues):
        ...
        self.cwba = CropWaterBalanceAccumulator(day, kiosk, parvalues)
